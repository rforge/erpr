...
scalp.subj.panel=function(panel)
	{
		scalp.subj.endo=function(base, numbers, outline, smo, col, startmsec, endmsec, yrev, ylims, lwd, lty, color.list=c("black","red"), layout, erplist=erplist){

		curr.subj=erplist[[paste(base, outline ,sep="")]]
  ...
  if (!is.null(scalp.array)){
	...
	text((dim(categ[[1]])[1]/2)+(dim(categ[[1]])[1]/8),0, labels=expression(paste(mu,"V")), cex=1.4)
			...
			if (electrodes[i]=="legend"){
				plot.new()
	legend("center", legend=paste(base, outline, sep=""), lty=lty, col=color.list, cex=1.2, lwd=lwd)
...
plot(1, type="n", frame.plot=FALSE,xlim=c(1,dim(categ[[1]])[1]),xaxt="n",yaxt="n", ylim=c(yedge[1]+yedge[1]/3,yedge[2]+(yedge[2]/3)))
	...
	#introduction to R" pag. 68. Vedi anche sotto.
		...
		scalp.endo(list(curr.subj),smo=smo,color.list=color.list, startmsec=startmsec, endmsec=endmsec, ylims=ylims, yrev=yrev, lwd=lwd, lty=lty, layout=layout)	
		}

		
		scalp.subj.endo(base=base, numbers=numbers, outline=panel$outnumber, layout=layout, smo=smo, ylims=ylims, yrev=yrev, startmsec=startmsec, endmsec=endmsec, color.list=c(col), lwd=lwd, lty=lty,erplist=erplist)
		
		panel
		
		}
		panel <- rpanel::rp.control() #se volessi creare piu pannelli allora dovrei aggiungere un'altro panel.
       rpanel::rp.listbox(panel, outnumber, numbers, labels=as.character(numbers), action = scalp.subj.panel, initval=numbers[1], title="Subject")
	   rpanel::rp.do(panel, scalp.subj.panel)
